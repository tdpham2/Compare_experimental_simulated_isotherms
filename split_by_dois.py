...
for item in nist_iso:
    ...
    for iso in isotherms:
        ...
        if len(adsorbates) == 1:
            ...
            if adsorbate_name not in interested_isotherms:
                continue
            else:
                subprocess.run('mkdir -p {}/{}'.format(output, doi), shell=True)
                subprocess.run('cp {} {}/{}/'.format(iso, output, doi), shell=True)
        else:
            print(iso)
            continue
